...
参考图交付获批后进入正式第六阶段：把每条风格唯一的最终参考图上传到 VidMuse dev 项目，验证可公开访问的 HTTPS 链接，并在独立目录中回填 `imageUrl`。
...
| 参考图地址（`imageUrl`） | 指向最终展示给用户的一张原创参考图 |
...
只上传阶段 5 已批准的最终参考图。Skill 使用 VidMuse dev 环境和 `Evals-bread-img` Plugin，按清单顺序生成可访问的 HTTPS 链接，并按风格序号、名称和文件名精确回填 `imageUrl`。

结果写入独立的 `06-url-backfill/`，不覆盖阶段 5 交付物；链接有效性及 JSON/CSV 对应关系通过校验后才可批准。

人工确认：图片、链接和最终六字段记录是否一一对应且可正常访问。
