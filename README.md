...
这是一套面向 AI Agent 的可复用 Skill，用于把公开网站、数据集或本地视觉素材，逐步整理成可审阅、可区分、可执行的 VidMuse 风格记录。
...
流程默认停在本地交付，不上传图片、不回填图片地址，也不写入线上后台。

## 适合谁

这套 Skills 适合：

- 需要从大量图片、影视截图、艺术作品、摄影、动画、游戏、建筑或混合媒介素材中批量发现风格的人；
- 需要扩充 VidMuse 风格库，但不了解现有聚类规则和字段规范的运营、产品、设计或研究人员；
- 希望让 AI 先完成采集、整理、预审和起草，再由人工在关键节点确认的人；
- 希望不同来源都使用相同质量标准，而不是每换一个网站就重新设计流程的人。

它不适合把颜色、柔光、情绪词或单张好看的图片直接包装成“新风格”。这些内容可以是风格特征，但通常不足以独立成为一种可用风格。
...
- 锚点层级是否不同且合理；
- 至少数个核心视觉维度是否稳定不同；
- 使用相同可比内容生成时，结果是否容易区分；
- 新候选是否真正增加了用户选择，而不是现有风格的别名；
- 父级风格和作品级叶子风格是否各自有足够证据。
...
每条通过审核的风格包含六类信息。下面先使用读者容易理解的名称，再在括号中给出导出文件里的系统键名：
...
第一轮确认前会查询所选环境的实时官方风格库，汇报当前可见构成和明显缺口。

这份结果只作为背景，不得改变本来源的采集范围、采集优先级、采样分层或停止条件。不同素材库的结构并不相同，不能用现有官方库的偏差反向限制新来源。

#### 权利信息

许可、署名、登录和发布限制需要如实记录，但 `research_only` 或权利未知不会自动终止研究采集。研究证据不能因此被直接当成用户参考图；参考图应另行生成原创内容。
...
电影、艺术、摄影、2D、3D、平面设计、建筑和连续视频可以使用不同的来源适配器，但最终都进入同一证据合同。
...
不是每种媒介都必须填写镜头、光线或运镜。只使用素材和适配器实际支持的观察维度。
...
可以完成来源调研、采集和证据整理。没有实时官方库时，正式概念批准会有重复风险；审核者需要明确接受该风险，或在恢复 CLI 后重新查重。
...
本仓库未附带开源许可证。仓库公开可见不代表自动授予复制、修改、分发或商业使用许可。
