...
| 参考图地址（`imageUrl`） | 指向最终展示给用户的一张原创参考图；本地交付阶段保持为空 |
...
总控入口。负责建立一次运行、管理五个阶段、冻结已确认结果、恢复中断任务，以及防止后续重跑覆盖人工决定。
...
负责把已确认的风格概念转成六类正式内容，进行近邻生成对照，并生成一张原创参考图及最终 JSON / CSV。
